...
Transform2D transform_to_2d(Transform from) {
	Vector3 fx	= from.basis[0];
	Vector3 fy	= from.basis[1];
	Vector3 fz	= from.basis[2];
	Vector3 fo	= from.origin;

	Transform2D tr = Transform2D(
		fx.x, fx.y,
		fy.x, fy.y,
		fo.x, -fo.y
	);
...
Transform transform_to_3d(Transform2D from, Transform aux) {
	...
	Vector3 ax	= aux.basis[0];
	Vector3 ay	= aux.basis[1];
	Vector3 az	= aux.basis[2];
	Vector3 ao	= aux.origin;
...
Vector3 xform_2d(Transform tr, Vector3 v) {
	Vector3 vv = Vector3(v.x, v.y, 0.0);
...
Ref<World2D> CollisionObjectSpatial2D::get_world_2d() const {
	ERR_FAIL_COND_V(!is_inside_world(), Ref<World2D>());
...
void CollisionObjectSpatial2D::set_collision_layer_bit(int p_bit, bool p_value) {
...
bool CollisionObjectSpatial2D::get_collision_layer_bit(int p_bit) const {
	ERR_FAIL_INDEX_V_MSG(p_bit, 32, false, "Collision layer bit must be between 0 and 31 inclusive.");
	return get_collision_layer() & (1 << p_bit);
}

void CollisionObjectSpatial2D::set_collision_mask_bit(int p_bit, bool p_value) {
...
bool CollisionObjectSpatial2D::get_collision_mask_bit(int p_bit) const {
...
void CollisionObjectSpatial2D::shape_owner_set_one_way_collision_margin(uint32_t p_owner, float p_margin) {
	if (area) {
		return; //not for areas
	}
...
float CollisionObjectSpatial2D::get_shape_owner_one_way_collision_margin(uint32_t p_owner) const {
...
Array CollisionObjectSpatial2D::_get_shape_owners() {
	Array ret;
	for (Map<uint32_t, ShapeData>::Element *E = shapes.front(); E; E = E->next()) {
		ret.push_back(E->key());
	}
...
void CollisionObjectSpatial2D::_input_event(Node *p_viewport, const Ref<InputEvent> &p_input_event, int p_shape) {
...
String CollisionObjectSpatial2D::get_configuration_warning() const {
	...
	if (shapes.empty()) {
		...
		warning += TTR("This node has no shape, so it can't collide or interact with other objects.\nConsider adding a CollisionShapeSpatial2D or CollisionPolygonSpatial2D as a child to define its shape.");
	}
...
void CollisionObjectSpatial2D::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_collision_layer_bit", "bit", "value"), &CollisionObjectSpatial2D::set_collision_layer_bit);
	ClassDB::bind_method(D_METHOD("get_collision_layer_bit", "bit"), &CollisionObjectSpatial2D::get_collision_layer_bit);
	ClassDB::bind_method(D_METHOD("set_collision_mask_bit", "bit", "value"), &CollisionObjectSpatial2D::set_collision_mask_bit);
	ClassDB::bind_method(D_METHOD("get_collision_mask_bit", "bit"), &CollisionObjectSpatial2D::get_collision_mask_bit);
